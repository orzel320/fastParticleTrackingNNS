...
def compute_pair_features(features_a: np.ndarray, features_b: np.ndarray) -> np.ndarray:
    ...
    cross_prod = (
        features_a[:, 1] * features_b[:, 0] - features_a[:, 0] * features_b[:, 1]
    )
    dot_prod_xy = (
        features_a[:, 0] * features_b[:, 0] + features_a[:, 1] * features_b[:, 1]
    )
    ...
    out[:, 6] = (
        features_a[:, 3] * features_b[:, 3] + features_a[:, 4] * features_b[:, 4]
    )
...
def create_pair_dataset(
    ...
    pair_features = compute_pair_features(
        dataset.X[final_queries], dataset.X[final_candidates]
    )
...
def split_by_event(
    ...
    if len(unique_events) < 3:
        raise ValueError(
            f"Zbyt mało zdarzeń ({len(unique_events)}) do poprawnego podziału na Train/Val/Test."
        )
