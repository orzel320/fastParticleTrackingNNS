...
class PipelineEvaluator:
    ...
    def _evaluate_predictions(
        self, query_idx: np.ndarray, neighbor_idx: np.ndarray, mask: np.ndarray
    ) -> dict:
        ...
        efficiency = (
            tp_count / self.true_pairs_count if self.true_pairs_count > 0 else 0.0
        )

        return {
            "Purity": purity,
            "Efficiency": efficiency,
            "Proposed_Pairs": proposed_count,
        }

    def run_geometric_pipeline(self, retriever, X_retrieval=None) -> dict:
        """Execute and evaluate a pipeline using KNN retrieval and hard geometric cuts.

        This method supports precise benchmarking by executing unmeasured warmup
        iterations followed by multiple measured runs to compute average timings.

        Args:
            retriever: An initialized nearest neighbor search object implementing
                `fit` and `kneighbors`.
            X_retrieval: Optional feature matrix used specifically for the index
                retrieval step. If None, defaults to `self.dataset.X`.

        Returns:
            A dictionary containing averaged timing breakdowns (Time_Retrieval_s,
            Time_Features_s, Time_Filter_s, Time_Total_s) and performance metrics
            (Purity, Efficiency, Proposed_Pairs) over the specified number of runs.
        """
        ...
        def _execute():
            ...
            features = compute_pair_features(
                self.dataset.X[query_idx_filtered],
                self.dataset.X[neighbor_idx_filtered],
            )
            ...
            dot_prod = features[:, 6]
            pass_cuts = (delta_r <= self.geom_cuts["max_delta_r"]) & (
                dot_prod >= self.geom_cuts["min_dot_product"]
            )
    ...
    def run_ml_pipeline(self, retriever, classifier, X_retrieval=None) -> dict:
        """Execute and evaluate a pipeline using KNN retrieval and a machine learning classifier.
        ...
        Returns:
            A dictionary containing averaged timing breakdowns (Time_Retrieval_s,
            Time_Features_s, Time_Filter_s, Time_Total_s) and final classification
            metrics over the specified number of runs.
        """
        X_search = X_retrieval if X_retrieval is not None else self.dataset.X
    ...
    def run_all_pairs_pipeline(self, classifier) -> dict:
        ...
        def _execute():
            ...
            q_grid, n_grid = np.meshgrid(
                np.arange(n_points), np.arange(n_points), indexing="ij"
            )
