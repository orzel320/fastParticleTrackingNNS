...
def plot_distance_distributions(
    dataset: TrackDataset, output_path: str = None, sample_size: int = 2000
):
...
def plot_pareto_frontier(
    df: pd.DataFrame, title: str = "Pareto Frontier", output_path: str = None
):
...
def plot_roc_curves(
    models_dict: dict, test_dataset: TrackDataset, output_path: str = None
):
...
def plot_pr_curves(
    ...
    for model_name, model in models_dict.items():
        ...
        axes.plot(
            recall, precision, lw=2, label=f"{model_name} (PR AUC = {pr_auc:.4f})"
        )
...
def plot_metric_dimension_heatmap(
    ...
    for ax, model in zip(axes, models):
        ...
        pivot = model_df.pivot_table(
            index="Dimension", columns="Size", values=metric, aggfunc="mean"
        )
        ...
        for i in range(len(pivot.index)):
            for j in range(len(pivot.columns)):
                ...
                if not np.isnan(val):
                    color = (
                        "white"
                        if (val < threshold and log_scale)
                        or (val > threshold and not log_scale and cmap != "RdYlGn")
                        or (val < threshold and cmap == "RdYlGn")
                        else "black"
                    )
                    ax.text(
                        j,
...
def plot_metric_lines_by_dimension(
    ...
    fig, axes = plt.subplots(
        1, len(dimensions), figsize=(5.5 * len(dimensions), 5), sharey=True
    )
    if len(dimensions) == 1:
        axes = [axes]
...
def plot_silver_bullet(df: pd.DataFrame, output_path: str = None):
    ...
    g.set_axis_labels(
        "Total Time per Event (seconds)", "Physics Quality (Purity x Efficiency)"
    )
    g.set_titles(col_template="Dataset: {col_name}")
    g.figure.suptitle(
        "The Silver-Bullet Plot: Physics Quality vs Total Time",
        y=1.05,
        fontsize=16,
        fontweight="bold",
    )
    ...
    sns.move_legend(
        g, "center left", bbox_to_anchor=(1.02, 0.5), title="Pipeline (X = Mean)"
    )
...
def plot_time_vs_size(df: pd.DataFrame, output_path: str = None):
    ...
    summary = (
        plot_df.groupby(["Pipeline", "Total_Size"])["Time_Total_s"].sum().reset_index()
    )
    ...
    plt.title(
        "Skalowalność: Całkowity czas przetwarzania vs Wielkość zbioru",
        fontsize=14,
        fontweight="bold",
    )
...
def plot_time_breakdown(df: pd.DataFrame, output_path: str = None):
    ...
    """
    summary = (
        df.groupby("Pipeline")[["Time_Retrieval_s", "Time_Features_s", "Time_Filter_s"]]
        .mean()
        .sort_values(by="Time_Retrieval_s")
    )
    ...
    plt.title(
        "Bottleneck Analysis: Average Time Breakdown per Pipeline",
        fontsize=14,
        fontweight="bold",
    )
...
def plot_purity_vs_efficiency(df: pd.DataFrame, output_path: str = None):
    ...
    summary_df = (
        df.groupby(["Dataset", "Pipeline"])
        .agg(Purity_Mean=("Purity", "mean"), Efficiency_Mean=("Efficiency", "mean"))
        .reset_index()
    )
    ...
    plt.title(
        "Physics Quality Trade-off: Purity vs Efficiency",
        fontsize=14,
        fontweight="bold",
    )
