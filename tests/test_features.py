...
def test_create_pair_dataset_excludes_noise_and_self_pairs():
    ...
    assert pairs.X.shape[0] == pairs.y.shape[0] == pairs.event_ids.shape[0]
    assert set(np.unique(pairs.y)).issubset({0, 1})
...
def test_create_pair_dataset_respects_max_neg_ratio_as_upper_bound():
    ...
    assert np.all(pairs.y == 1)
    assert pairs.X.shape[0] == n_hits
...
def test_split_by_event_no_overlap_between_splits():
    ...
    dataset = TrackDataset(X=X, y=y, event_ids=event_ids)

    train_ds, val_ds, test_ds = split_by_event(
        dataset, train_size=0.7, val_size=0.15, seed=42
    )

    train_set = set(np.unique(train_ds.event_ids))
    val_set = set(np.unique(val_ds.event_ids))
    test_set = set(np.unique(test_ds.event_ids))
